...
<line x1="${BAR_X}" y1="${BY2}" x2="${BAR_X}" y2="${BY2 + BH}" stroke="${c}" stroke-width="2.5"/>`;
  ...
  function _getDxfBounds() {
    ...
    for (const e of allEntities) {
      const pts = e.t === "L" ? [[e.x0, e.y0], [e.x1, e.y1]] : e.t === "P" ? (e.verts || []).map((v) => [v.x, v.y]) : e.t === "C" || e.t === "A" ? [[e.cx - e.r, e.cy], [e.cx + e.r, e.cy], [e.cx, e.cy - e.r], [e.cx, e.cy + e.r]] : [];
    ...
    if (!customBounds) {
      ...
      if (dxfB) {
        if (!bounds) bounds = dxfB;
          ...
          bounds = { minX: Math.min(bounds.minX, dxfB.minX), minY: Math.min(bounds.minY, dxfB.minY), maxX: Math.max(bounds.maxX, dxfB.maxX), maxY: Math.max(bounds.maxY, dxfB.maxY), isBg: bounds.isBg };
        }
      }
    }
      ...
      try {
        ...
        svgEl.querySelectorAll("text, tspan").forEach((t) => {
          ...
          if (t.childElementCount === 0) {
            t.textContent = t.textContent.replace(/Ă/g, "A").replace(/ă/g, "a").replace(/Ș/g, "S").replace(/ș/g, "s").replace(/Ț/g, "T").replace(/ț/g, "t");
          }
        });
...
`;
  ...
  function onDn(e) {
    ...
    if (tg || hb) return;
    if (S.mode !== "select" && S.mode !== "connect") setMode("select");
    if (S.mode === "connect" && !S.connStart) setMode("select");
    if (S.mode === "select") {
...
<p style="font-family:Calibri,sans-serif;font-size:10pt">${consText}</p>
  ...
  function renderVDOverlay() {
    ...
    byEl.forEach((dataList, elId) => {
      ...
      if (el.type.startsWith("cd") || el.type.startsWith("ptab_") || el.type === "trafo" || el.type === "celula_linie_mt" || el.type === "celula_trafo_mt" || el.type === "bara_mt" || el.type === "bara_statie_mt" || el.type === "manson" || el.type === "meter") return;
  ...
  function render() {
    ...
    S.EL.forEach((el) => {
      ...
      if (re.type === "polyline") {
        ...
        g2.innerHTML = `<defs>${markersDef}</defs><polyline points="${pts}" fill="none" stroke="${re.color || "#00cfff"}" stroke-width="${sw}" ${dash} ${markersAttr}/>`;
...
- ${tipCC} tip ${cableName} in lungime de ${len.toFixed(0)} m pana la un ${bmptLabel} montat ${getBMPTLocation(len, isFirida)}.`;
  ...
  var TYPE_RO = {
    ...
    cd8: "CD 8P",
    stalp_se4: "stalp SE4",
    stalp_se10: "stalp SE10",
    stalp_sc10002: "stalp SC10002",
    stalp_sc10005: "stalp SC10005",
    stalp_rotund: "stalp rotund lemn",
    firida_e2_4: "Firida E2-4",
    firida_e3_4: "Firida E3-4",
    firida_e3_0: "Firida E3-0",
    firida_gen: "Firida Generala",
    meter: "BMPT"
  };
  function buildSchemaSummary() {
    ...
    const L = [];
    const sources = S.EL.filter(
      (e) => e.type === "trafo" || e.type.startsWith("ptab_") || e.type.startsWith("cd")
    );
    if (sources.length) {
      L.push("SURSE (PT / CD):");
      sources.forEach((e) => {
    ...
    if (circuitCN.length) {
      ...
      Object.entries(byTip).forEach(([tip, cables]) => {
        ...
        cables.forEach((cn) => {
          ...
          let note = "";
          if (fTag === "[EXISTENT]" && tTag === "[EXISTENT]") note = "  \u2190 conductor NOU pe st\xE2lpi EXISTEN\u021AI";
          else if (fTag === "[NOU]" || tTag === "[NOU]") note = "  \u2190 conductor NOU pe st\xE2lpi NOI";
    ...
    if (bransamenteCN.length) {
      ...
      bransamenteCN.forEach((cn) => {
        ...
        const locatie = srcIsFirida || len > 8 ? "pe soclu de beton la limita de proprietate" : "pe stalpul de racord";
    ...
    if (rInt.length || intarirCN.length) {
      L.push("\nLUCR\u0102RI \xCENT\u0102RIRE RE\u021AEA:");
      intarirCN.forEach((cn) => {
        const f = elById[cn.fromElId], t = elById[cn.toElId];
        if (cn.stare === "intarire_inlocuire" && cn.oldTipConductor) {
          L.push(`  \xCEnlocuire ${cn.oldTipConductor} ${cn.oldSectiune || "?"}mmp \u2192 ${cn.tipConductor} ${cn.sectiune}mmp L=${cn.length || 0}m [${elName(f)} \u2192 ${elName(t)}]`);
        } else {
...
6b. Lucr\u0103ri \xCEnt\u0103rire Re\u021Bea (draft):
...
Returneaz\u0103 un JSON cu 3 c\xE2mpuri \u2014 textul corectat \u0219i completat fa\u021B\u0103 de draft:
{
  "racordare": "<sec\u021Biunea 6a corectat\u0103>",
  "intarire": "<sec\u021Biunea 6b corectat\u0103 sau NU ESTE CAZUL>",
  "retea": "<sec\u021Biunea 5 corectat\u0103>"
}
R\u0103spunde DOAR cu JSON valid, f\u0103r\u0103 text \xEEn afara JSON-ului.`;
    ...
    try {
      const parsed = prov === "gemini" ? await _callGemini(systemPrompt, userPrompt, key) : await _callGroq(systemPrompt, userPrompt, key);
      const set = (id, val) => {
        const el = document.getElementById(id);
        if (el) el.value = val || "";
      };
  ...
  function _initTouch(svgEl) {
    ...
    function fakeEv(type, touch, orig) {
      return {
        ...
        ctrlKey: orig.ctrlKey || false,
        metaKey: orig.metaKey || false,
    ...
    svgEl.addEventListener("touchstart", (e) => {
      ...
      if (e.touches.length === 2) {
        ...
        _pinchDist = dist2(e);
        if (_touchDragging) {
          onUp(fakeEv("mouseup", e.touches[0], e));
